...
DROP DATABASE IF EXISTS klant;
CREATE DATABASE klant;
USE klant;
--
...
DROP TABLE IF EXISTS klant.land;

CREATE TABLE `klant`.`land` (
                                id INT NOT NULL AUTO_INCREMENT,
                                naam VARCHAR(45) NOT NULL,
                                PRIMARY KEY (id),
                                UNIQUE INDEX naam_uk (naam));

insert into klant.land
values(1,'Suriname');

ALTER TABLE `klant`.`contact_informatie`
...
ALTER TABLE `klant`.`contact_informatie`
    ADD CONSTRAINT `land_fk`
        FOREIGN KEY (`land_id`)
            REFERENCES `klant`.`land` (`id`);

drop table if exists klant;
create table klant(id int not null auto_increment, bedrijfsnaam CHAR(50) NOT NULL, gebruikersnaam CHAR(50) NOT NULL,  klantnummer int NOT NULL, postcode CHAR(50) NOT NULL, PRIMARY KEY(id), persoonID int not null,
                   CONSTRAINT persoon_fk foreign key(persoonID) references persoon(id));
